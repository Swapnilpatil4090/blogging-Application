...
export const createPost = (postData,userData) => {
  
  return privateAxios
    .post(
      `/user/${userData.userId}/category/${postData.categoryId}/posts`,
      postData
    )
...
export const createComment = (comment, postId) => {
  return privateAxios.post(`/posts/${postId}/comment`, comment);
};
...
export function searchPost(keyword){
  return myAxios.get(`/posts/search/${keyword}`).then((resp) => resp.data);
}
